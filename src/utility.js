...
const getRequiredLines = function(fileContent, output) {
  const lines = fileContent[0];
...
const loadLinesFromFile = function(parsedUserInputs, helperFunctions) {
  ...
  return getRequiredLines(fileContent, helperFunctions.output);
};

const handleSubOperations = function(valid, parsedUserInputs, helperFunctions) {
  if (valid == false) return;
  if (parsedUserInputs.fileNames.length == 0)
    return "waiting for standard input";
  return loadLinesFromFile(parsedUserInputs, helperFunctions);
};
...
const parseUserArguments = function(commandLineArgs, error) {
  ...
  if (optionOrFile.includes("-")) {
    ...
    if (userArgs[1] != undefined && Number.isNaN(+userArgs[1])) {
      error(`tail: illegal offset -- ${userArgs[1]}`);
      return { valid: false };
    }
  }
